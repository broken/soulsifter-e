...
module Attrib
  ...
  MULTIFIND = 2**9 # field: add a find method that returns an iterator for the field
end
...
def hSecondaryFindFunctions(name, multifindFields)
  str = ""
  multifindFields.each do |f|
...
def cSecondaryFindFunctions(name, multifindFields,  fields)
  str = ""
  multifindFields.each do |f|
...
def writeHeader (name, fields, attribs, customMethods, customHeaders)
  ...
  multifindFields = fields.select{|f| f[2] & Attrib::MULTIFIND > 0 }
  if (secondaryKeys.length > 1)
    multifindFields = multifindFields + secondaryKeys
  end
  str = ""
  ...
  str << hSecondaryFindFunctions(name, multifindFields)
...
def writeCode (name, fields, attribs)
  ...
  str << cSecondaryFindFunctions(name, multifindFields, fields)
